...
subject to elec_prod_PT_POWER_BLOCK: sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['PT_POWER_BLOCK', h , td] ) = 22184.7;#GWh;
#fix F ['PT_POWER_BLOCK'] :=2.2225;#GW
subject to elec_prod_ST_POWER_BLOCK: sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['ST_POWER_BLOCK', h , td] ) = 805.113;#GWh;
#fix F ['ST_POWER_BLOCK'] :=0.051;#GW
subject to elec_prod_STIRLING_DISH: sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['STIRLING_DISH', h , td] ) = 10.17699;#GWh;
...
subject to biogas_prod_BIOMETHANATION: sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['BIOMETHANATION', h , td] ) =10400;#GWh;

subject to f_min_perc_cars :
	sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t], j in {'CAR_HEV','CAR_BEV'} } (F_t [j,h,td] * t_op[h,td]) >= 0.1 * sum {j2 in TECHNOLOGIES_OF_END_USES_TYPE['MOB_PRIVATE'], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (F_t [j2, h, td] * t_op[h,td]);
subject to f_max_perc_cars :
	sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t], j in {'CAR_HEV','CAR_BEV'} } (F_t [j,h,td] * t_op[h,td]) <= 0.1 * sum {j2 in TECHNOLOGIES_OF_END_USES_TYPE['MOB_PRIVATE'], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (F_t [j2, h, td] * t_op[h,td]);


#let fmin_perc['CAR_BEV'] + let fmin_perc['CAR_HEV'] := 0.1;
#let fmax_perc['CAR_BEV'] + let fmax_perc['CAR_HEV']:= 0.1;
...
/*
let fmin_perc['NUCLEAR'] := 0.235105;
let fmax_perc['NUCLEAR'] := 0.235105;
let fmin_perc['WIND_ONSHORE'] := 0.206616;
let fmax_perc['WIND_ONSHORE'] := 0.206616;
let fmin_perc['CCGT'] := 0.153948;
let fmax_perc['CCGT'] := 0.153948;
let fmin_perc['COAL_US'] := 0.226663;
let fmax_perc['COAL_US'] := 0.226663;
let fmin_perc['COAL_IGCC'] := 0;
let fmax_perc['COAL_IGCC'] := 0;
let fmin_perc['PV'] := 0.035363;
let fmax_perc['PV'] := 0.035363;
let fmin_perc['WIND_OFFSHORE'] := 0;
let fmax_perc['WIND_OFFSHORE'] := 0;
let fmin_perc['HYDRO_RIVER'] := 0.035355;
let fmax_perc['HYDRO_RIVER'] := 0.035355;
let fmin_perc['HYDRO_DAM'] := 0.085115;
let fmax_perc['HYDRO_DAM'] := 0.085115;
let fmin_perc['GEOTHERMAL'] := 0;
let fmax_perc['GEOTHERMAL'] := 0;
let fmin_perc['WAVE'] := 0.000001;
let fmax_perc['WAVE'] := 0.000001;
let fmin_perc['PT_POWER_BLOCK'] := 0.02106;
let fmax_perc['PT_POWER_BLOCK'] := 0.02106;
let fmin_perc['ST_POWER_BLOCK'] := 0.000764;
let fmax_perc['ST_POWER_BLOCK'] := 0.000764;
let fmin_perc['STIRLING_DISH'] := 0.00001;
let fmax_perc['STIRLING_DISH'] := 0.00001;



##Emissions 2015
let gwp_op["COAL"] := 0.36;#0.354;# gwp_direct =0.36;
...
let fmin_perc['DHN_COGEN_WOOD'] := 0.945518;
let fmax_perc['DHN_COGEN_WOOD'] := 0.945518;
...
let end_uses_demand_year ["ELECTRICITY", "HOUSEHOLDS"]:=715.37; #59619.47;#GWh 66129.55;
let end_uses_demand_year ["ELECTRICITY", "INDUSTRY"]:=1580.796; #3040.51;#GWh 74106.74;
let end_uses_demand_year ["ELECTRICITY", "SERVICES"]:=863.11; #48822.90;#GWh 55513.46;
let end_uses_demand_year ["ELECTRICITY", "TRANSPORTATION"]:=0;#GWh
let end_uses_demand_year ["LIGHTING", "HOUSEHOLDS"]:=44560.96; #4173.62;#GWh 8892.67;
let end_uses_demand_year ["LIGHTING", "INDUSTRY"]:=98469.36; #6446.006055;#GWh 7329.24;
let end_uses_demand_year ["LIGHTING", "SERVICES"]:=53764.18; #13207.29;#GWh 21588.57; 
let end_uses_demand_year ["LIGHTING", "TRANSPORTATION"]:=0;#GWh
let end_uses_demand_year ["HEAT_HIGH_T", "HOUSEHOLDS"]:=0;#GWh
let end_uses_demand_year ["HEAT_HIGH_T", "INDUSTRY"]:=91758.76; #114681.6647;#GWh
let end_uses_demand_year ["HEAT_HIGH_T", "SERVICES"]:= 0; #4439.598586;#GWh
let end_uses_demand_year ["HEAT_HIGH_T", "TRANSPORTATION"]:=0;#GWh
let end_uses_demand_year ["HEAT_LOW_T_SH", "HOUSEHOLDS"]:=93157.8; #49217.925706259;#GWh 60787.73;
let end_uses_demand_year ["HEAT_LOW_T_SH", "INDUSTRY"]:=23866.49; #22269.59981;#GWh
let end_uses_demand_year ["HEAT_LOW_T_SH", "SERVICES"]:=70520.27; #28792.7528;#GWh
let end_uses_demand_year ["HEAT_LOW_T_SH", "TRANSPORTATION"]:=0;#GWh
let end_uses_demand_year ["HEAT_LOW_T_HW", "HOUSEHOLDS"]:=23812.27; #43324.60398;#GWh
let end_uses_demand_year ["HEAT_LOW_T_HW", "INDUSTRY"]:=6183.34; #0;#GWh
let end_uses_demand_year ["HEAT_LOW_T_HW", "SERVICES"]:=15808.41; #4422.54503;#GWh
let end_uses_demand_year ["HEAT_LOW_T_HW", "TRANSPORTATION"]:=0;#GWh
let end_uses_demand_year ["PROCESS_COOLING", "HOUSEHOLDS"]:=0;#GWh
let end_uses_demand_year ["PROCESS_COOLING", "INDUSTRY"]:=15000; #15438.63658;#GWh
let end_uses_demand_year ["PROCESS_COOLING", "SERVICES"]:=13000; #9552.047114;#GWh
let end_uses_demand_year ["PROCESS_COOLING", "TRANSPORTATION"]:=0;#GWh
let end_uses_demand_year ["SPACE_COOLING", "HOUSEHOLDS"]:=16837.23; #11951.30749;#GWh
let end_uses_demand_year ["SPACE_COOLING", "INDUSTRY"]:=14000; #11686.50279;#GWh
let end_uses_demand_year ["SPACE_COOLING", "SERVICES"]:=41162.77; #34487.71344;#GWh
...
let end_uses_demand_year ["MOBILITY_PASSENGER", "TRANSPORTATION"]:=568000; #418901;#Mpkm 418901
...
let end_uses_demand_year ["MOBILITY_FREIGHT", "TRANSPORTATION"]:=282000; #263912;#Mtkm;
...
let fmin_perc ['IND_COGEN_GAS'] := 0.275489; #0.471;#0.4712508; #0.275489;
let fmax_perc ['IND_COGEN_GAS'] := 0.275489; #0.471;#0.4712508; #0.275489;
...
let fmin_perc ['IND_COGEN_WASTE'] := 0.022237; #0.103;#0.064752; #0.022237;
let fmax_perc ['IND_COGEN_WASTE'] := 0.022237; #0.103;#0.064752; #0.022237;
