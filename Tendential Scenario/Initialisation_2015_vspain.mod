...
subject to elec_prod_PT_POWER_BLOCK: sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['PT_POWER_BLOCK', h , td] ) = 5409.211;#GWh;
#fix F ['PT_POWER_BLOCK'] :=2.2225;#GW
subject to elec_prod_ST_POWER_BLOCK: sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['ST_POWER_BLOCK', h , td] ) = 196.3075;#GWh;
#fix F ['ST_POWER_BLOCK'] :=0.051;#GW
subject to elec_prod_STIRLING_DISH: sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['STIRLING_DISH', h , td] ) = 2.4814;#GWh;
...
subject to elec_prod_BIOMETHANATION: sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['BIOMETHANATION', h , td] ) =10400;#GWh;


subject to f_min_perc_cars :
	sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t], j in {'CAR_HEV','CAR_BEV'} } (F_t [j,h,td] * t_op[h,td]) >= 0.05 * sum {j2 in TECHNOLOGIES_OF_END_USES_TYPE['MOB_PRIVATE'], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (F_t [j2, h, td] * t_op[h,td]);
